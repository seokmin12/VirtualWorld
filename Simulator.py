...
class Simulator:
    ...
    def mine(self, entity):
        ...
        self._update_condition(mining_time, entity)

    def _update_condition(self, elapsed_time, entity):
        """채굴 시간에 따른 컨디션 변화"""
        condition_change_chance = min(0.8, elapsed_time / 3600)  # 최대 80% 확률로 컨디션 변화
        
        if random.random() < condition_change_chance:
            current_index = list(entity.conditions.keys()).index(entity.current_condition)
            if current_index < 2:  # high -> medium -> low
                entity.current_condition = list(entity.conditions.keys())[current_index + 1]
                # print(f"컨디션이 {self.user.current_condition}로 변화했습니다.")

    def rest(self, entity):
        """휴식을 취하고 체력을 회복"""
        recovery = entity.rest_recovery_rate
        entity.health = min(100, entity.health + recovery)
    ...
    def choose_action(self, entity):
        ...
        if entity.should_rest():
            self.rest()
        else:
            # 체력과 컨디션에 따른 채굴 확률 계산
            mining_chance = 0.7  # 기본 70% 확률로 채굴
            if entity.current_condition == 'high':
                mining_chance = 0.9
            elif entity.current_condition == 'low':
                mining_chance = 0.5
            
            if random.random() < mining_chance or entity.health == 100:
                self.mine()
            else:
                self.rest()
    ...
    def run(self, entity):
        while True:
            self.choose_action()
            if entity.health == 0.00 or self.time >= 3600 * 24:
                print(f"{self.days + 1}일차 잔액 돈: {entity.account.balance}")
                break
        self.days += 1
        self.time = 0
